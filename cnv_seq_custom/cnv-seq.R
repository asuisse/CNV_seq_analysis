...
file.cleanR <- function(cnv_file) {
    ...
    cat("Filtering on number of reads per window (each window must have more than 50 reads in cont sample)", "\n")
    clean_file <- filter(clean_file, ref > 50)
...
cnv.print(clean_file, file=paste(id,  win, 'cnvs.txt', sep='_'))
